...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    b = np.sum(np.square(X),axis = 1, keepdims = True)
    product = np.dot(X,self.X_train.T)
    dists = np.sqrt(a + b - 2 * product)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      ids = np.argsort(dists[i])[:k]

      for num in ids:
      	closest_y.append(self.y_train[num])
      ...
      for label in closest_y:
      	if label not in tracked:
      		...
      		tracked[label] +=1
      highest = 0
      for label,frequency in tracked.items():
      	if highest == 0:
      		highest = frequency
      		y_pred[i] = label
